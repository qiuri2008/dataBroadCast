...
static int _convert_24_to_16(unsigned char *src, FILE *fp, unsigned int size)
{
    int i = 0, ret = 0;
...
int bmp_24bpp_to_16bpp(void *data, int length, char *filename)
{
    int ret = 0, i = 0;
    ...
    if(24 != bpp)
    {
	printf("The source BMP is not 24 bit per pixel BMP file, please check!\n");
	return (2);
    }
    ...
    ptr = ptr + offset;
    hoffset = (((width * 24) + 31) >> 5) << 2;

    for(i = 0; i < height; i++)
    {
	_convert_24_to_16(ptr + i * hoffset, fp, width);	
    }
