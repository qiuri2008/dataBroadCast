							...
							int qsize, PTsFilterCallback pProc, uint32_t lParam  )
{
    ...
    SFilterNode *node = new SFilterNode();
    unsigned char *buf = new byte[qsize]();
	if( node )
	{
...
static bool Demux_ParseTs188( void* handle, SFilterNode *filter, unsigned char *data )
{
    ...
    case 0x10:/* 无负载 */
		return true;
    case 0x11: /* 有调整字段和负载 */
        payload_pos = data + 5 + data[4];/* 跳过adaptation_field data[4]为自适应于长度*/
		break;
	}

    /* 跳过pointer_field 负载单元开始标志
        为1时，在前4个字节之后会有一个调整字节，它的数值决定了负载内容的具体开始位置。
        现在看例子中的数据47 40 00 17 00第五个字节是00，说明紧跟着00之后就是具体的负载内容
        packet不满188字节时需填充在packet中间
    */
	if( data[1] & 0x40 )
    {
		p_new_pos = payload_pos + *payload_pos + 1;
        payloadLenth = *payload_pos;
